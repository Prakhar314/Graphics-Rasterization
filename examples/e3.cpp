...
int main() {
    ...
    while (!r.shouldQuit()) {
        ...
        // Rotated triangle.
        mvp = mat4(1.0f);
        mvp = rotate(mvp, (float)radians(45.0f), normalize(vec3(1.0f, 1.0f, 0.0f)));
        ...
        // Translated triangle.
        mvp = mat4(1.0f);
        mvp = translate(mvp, vec3(-0.1f, 0.0f, 0.0f));
        r.setUniform(program, "transform", mvp);
        r.setUniform<vec4>(program, "color", vec4(0.6, 0.0, 0.0, 1.0));
		r.drawObject(box);
